...
class TimelapseConverter:
    ...
    # Supported list data types
        ...
        # Map LayerDataTuples to the correct napari.types
        self.tuple_aliases = {
        ...
        self.supported_list_data = list(
            self.list_to_data_conversion_functions.keys())
        self.supported_4d_data = list(
    ...
    def convert_4d_data_to_list(self, data, layertype: type) -> list:
        ...
        if layertype not in self.supported_4d_data:
            raise TypeError(f'{layertype} data to list conversion ' +
    ...
    def convert_list_to_4d_data(self, data, layertype: type) -> list:
        ...
        if layertype not in self.supported_list_data:
            raise TypeError(f'{layertype} list to data conversion ' +
                            'currently not supported.')
    ...
    def _image_to_list_of_images(
            self, image: 'napari.types.ImageData') -> list:
        """Convert 4D image to list of images"""
        while len(image.shape) < 4:
            image = image[np.newaxis, :]
        return list(image)

    def _list_of_images_to_image(
            self, images: list) -> 'napari.types.ImageData':
    ...
    def _list_of_points_to_points(self, points: list) -> np.ndarray:
        ...
        dataframes = [
            pd.DataFrame(frame, columns=['z', 'y', 'x']) for frame in points
            ]
    ...
    def _points_to_list_of_points(
            self, points: 'napari.types.PointsData') -> list:
        ...
        points_out = [
            group[['z', 'y', 'x']].values for _, group in df.groupby('t')
            ]
    ...
    def _vectors_to_list_of_vectors(
            self, vectors: 'napari.types.VectorsData') -> list:
        ...
        output_vectors = [
            np.stack([pt, vec]).transpose((1, 0, 2)) for pt, vec in zip(
                point_list, vector_list)
                ]
        return output_vectors

    def _list_of_vectors_to_vectors(
            self, vectors: list) -> 'napari.types.VectorsData':
    ...
    def _surface_to_list_of_surfaces(
            self, surface: 'napari.types.SurfaceData') -> list:
        ...
        points_out = [
            group[['z', 'y', 'x']].values for _, group in df_points.groupby('t')
            ]
        ...
        faces_out = [
            group[['p1', 'p2', 'p3']].values for _, group in df_faces.groupby('t')
            ]
        ...
        if has_values:
            ...
            values_out = [
                group['values'].values for _, group in df_values.groupby('t')
                ]
        ...
        if has_values:
            surfaces_out = [
                (p, f, v) for p, f, v in zip(points_out, faces_out, values_out)
                ]
        else:
